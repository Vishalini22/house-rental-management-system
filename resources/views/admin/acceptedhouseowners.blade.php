<!-- CHANGES: 
1. Line: md:items-start (to top-align sections)
2. Line: <div class="pt-1"> (to slightly nudge title downward for alignment)
-->
...
<head>
  <meta charset="UTF-8" />
  <title>Approved House Owners - Rentalz Admin</title>
...
<div class="flex min-h-screen">
  ...
  <main class="flex-1 p-8 overflow-auto">
    <h2 class="text-3xl font-bold text-gray-800 mb-6">Approved House Owners</h2>
    ...
    <div class="space-y-6">
      @forelse($acceptedOwners as $owner)
        <div class="bg-white shadow rounded-md px-6 py-5 flex flex-col md:flex-row md:items-start md:justify-between gap-6">
          <!-- Profile & name -->
          <div class="flex items-center gap-4 w-full md:w-auto">
            @if($owner->profile_photo && file_exists(public_path('storage/' . $owner->profile_photo)))
              <img src="{{ asset('storage/' . $owner->profile_photo) }}" alt="Profile"
            ...
            @endif
            <div class="pt-1">
              <p class="font-semibold text-gray-900 text-base">{{ $owner->name }}</p>
              <p class="text-xs text-gray-500">{{ $owner->created_at->format('d M Y, h:i A') }}</p>
            </div>
          </div>

          <!-- Inline Info -->
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 flex-1 text-sm">
            <div>
              <p class="text-gray-400 uppercase text-xs font-semibold tracking-wide">Email</p>
              <p class="text-gray-900 break-words">{{ $owner->email }}</p>
            </div>
            <div>
              <p class="text-gray-400 uppercase text-xs font-semibold tracking-wide">Phone</p>
              <p class="text-gray-900 break-words">{{ $owner->phone ?? 'N/A' }}</p>
            </div>
            <div>
              <p class="text-gray-400 uppercase text-xs font-semibold tracking-wide">Location</p>
              <p class="text-gray-900 break-words">{{ $owner->address ?? 'Unknown' }}</p>
            </div>
            <div>
              <p class="text-gray-400 uppercase text-xs font-semibold tracking-wide">Business</p>
          ...
          <div class="text-sm">
            @if($owner->id_proof && file_exists(public_path('storage/' . $owner->id_proof)))
              @php
                $ext = pathinfo($owner->id_proof, PATHINFO_EXTENSION);
                $proofUrl = asset('storage/' . $owner->id_proof);
              @endphp
              <button 
                @click="modalOpen = true; modalContent = '{{ $proofUrl }}'; modalType = '{{ strtolower($ext) === 'pdf' ? 'pdf' : 'image' }}';"
                class="text-indigo-800 hover:text-indigo-900 font-semibold underline"
              >
                Show ID
              </button>
            @else
      ...
      @empty
        <p class="text-center text-gray-600">No approved house owners available.</p>
      @endforelse
    </div>
  </main>
</div>
...
<!-- Modal -->
<div x-show="modalOpen" x-transition style="display: none;"
