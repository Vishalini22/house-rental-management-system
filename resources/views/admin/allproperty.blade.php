...
<head>
  ...
  <title>Rentalz Admin - All Properties</title>
  ...
  <div class="flex min-h-screen">
      ...
      <nav class="space-y-2">
        ...
        <div>
          ...
          >
            ...
            <svg :class="{'transform rotate-180': openMenus.properties}" class="w-4 h-4 transition-transform" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round">
              <path d="M6 9l6 6 6-6"/>
            </svg>
          </button>
          <div x-show="openMenus.properties" x-transition class="mt-1 pl-4 space-y-1" style="display: none;">

            <!-- Approved - non-clickable -->
            <div
              class="block text-sm text-gray-400 font-medium px-3 py-1.5 cursor-default select-none rounded"
              aria-disabled="true"
              title="Already on Approved Properties"
            >
              Approved Properties
            </div>
        ...
        <div>
          ...
          >
            <span>Customers</span>
            <svg :class="{'transform rotate-180': openMenus.customers}" class="w-4 h-4 transition-transform" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round">
              <path d="M6 9l6 6 6-6"/>
            </svg>
          </button>
        ...
        <div>
          ...
          >
            ...
            <svg :class="{'transform rotate-180': openMenus.owners}" class="w-4 h-4 transition-transform" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round">
              <path d="M6 9l6 6 6-6"/>
            </svg>
          </button>
    ...
    <main class="flex-1 p-6 bg-gray-50 overflow-y-auto">
      <div class="mb-6 flex justify-between items-center">
        <h2 class="text-2xl font-bold text-gray-800">All Approved Properties</h2>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse ($properties as $property)
          <div class="bg-white rounded-xl shadow-md p-5 border border-gray-200 flex flex-col justify-between">
            <!-- Content wrapper -->
            <div class="flex-grow">
              <!-- House Image -->
              @if($property->image)
              ...
              <div class="flex items-center space-x-3 mb-4">
                @if($property->owner_photo)
                 <img src="{{ asset('storage/' . $property->owner_photo) }}" alt="Owner Photo" class="w-10 h-10 rounded-full object-cover object-top">
            ...
            <div class="flex justify-end">
              <a href="{{ route('admin.properties.approved.show', $property->id) }}" class="text-sm text-white bg-blue-600 hover:bg-blue-700 px-3 py-1 rounded">
                View Details
              </a>
            </div>
          </div>
